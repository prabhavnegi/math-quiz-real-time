...
class MathProblemGenerator {
    static generate() {
        const problemTypes = ['arithmetic', 'percentage', 'factors'];
        ...
        switch (selectedType) {
            ...
            case 'factors':
                return this.generateFactorsProblem();
            default:
    ...
    static generateFactorsProblem() {
        const primes = [2, 3, 5, 7, 11, 13];
        const numFactors = this.getRandomInt(2, 4);
        let number = 1;
        const usedFactors = [];
        for (let i = 0; i < numFactors; i++) {
            const factor = this.getRandomItem(primes);
            usedFactors.push(factor);
            number *= factor;
        }
        const question = `What is the largest factor of ${number}?`;
        const answer = Math.max(...usedFactors);
        return { question, answer };
    }
